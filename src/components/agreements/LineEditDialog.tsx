...
import {
  undistinguishedSiblings as computeUndistinguishedSiblings,
  clientDistinguishes as clientDistinguishesPure,
} from "@/lib/sku-conflict";
import {
  Dialog,
  DialogContent,
  DialogDescription,
    ...
    const t = setTimeout(async () => {
      try {
        ...
        if (exact && exact.status.kind === "taken") {
          ...
          // El código tecleado ya no matchea un código tomado: limpiar el
          // panel para que se recalcule con cada cambio del campo.
          setTakenBlock(null);
        }
      } catch (e) {
        if (s !== matchSeq.current) return;
...
export function LineEditDialog({
  ...
  const clientDistinguishes = (cid: string, siblingCode: string | null): boolean =>
    clientDistinguishesPure(cid, siblingCode, codeEntries, codeModes);
