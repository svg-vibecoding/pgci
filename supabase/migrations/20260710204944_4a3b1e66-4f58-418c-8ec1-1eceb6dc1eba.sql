...
-- 1. Reemplazar _resolve_client_code con nueva firma (p_brand, p_agreement_id)
DROP FUNCTION IF EXISTS public._resolve_client_code(uuid, text, text, uuid, text);
...
begin
  ...
  select cph.description, cph.brand
    into v_last_desc, v_last_brand
    from public.client_product_history cph
   where cph.client_product_id = v_cp
     and cph.valid_until is null
   order by cph.valid_from desc, cph.created_at desc
   limit 1;

  v_changed := (p_description is not null and p_description is distinct from v_last_desc)
            or (p_brand       is not null and p_brand       is distinct from v_last_brand);
  ...
  if v_changed then
    ...
    insert into public.client_product_history
      (client_product_id, description, brand, source, source_reference, valid_from)
    values
      (v_cp, p_description, p_brand, p_source::code_source, p_agreement_id, current_date);
  end if;
  ...
  if p_product_id is not null then
    ...
    if v_match is null then
      insert into public.client_product_match
           ...
           values (v_cp, p_product_id, current_date, p_source::code_source, auth.uid())
...
-- 2. create_agreement_line: pasar 'agreement' + p_agreement_id a _resolve_client_code
...
-- 3. update_agreement_line: pasar 'agreement' + v_agreement_id
...
begin
  ...
  if v_has_codes then
    ...
    for r in select * from jsonb_array_elements(v_codes) loop
      ...
      select client_product_id, client_product_match_id into v_cp, v_match
        from public._resolve_client_code(v_client_id, v_client_code, v_row_desc,
                                         v_row_tr.product_id, 'agreement', null, v_agreement_id);
