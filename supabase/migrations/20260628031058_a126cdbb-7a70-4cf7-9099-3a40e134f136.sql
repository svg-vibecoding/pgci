...
begin
  ...
  for v_row in select * from jsonb_array_elements(v_rows) loop
    ...
    -- Resolver línea existente: por match si existe, sino por (product_id) sin match.
    ...
    if v_existing_line_id is null then
      insert into public.agreement_products (
        agreement_id, product_id, client_product_match_id,
        sale_price, par_price, start_date, end_date,
        observations, created_by, updated_by
      ) values (
        p_agreement_id, v_product_id, v_match_id,
  ...
  -- Propagación N:1 "applyAll": para cada SKU marcado, toma el precio máximo
  -- visto en las filas importadas de ese SKU y lo aplica a TODAS las líneas
  -- no-excluidas del mismo SKU en el acuerdo.
