...
declare
  ...
  r jsonb;
  v_has_other boolean;
  v_codeless_exists boolean;
  v_clients_with_code uuid[];
  v_new_code_clients uuid[];
  v_client_names text;
begin
  ...
  -- GUARD por cliente: SKU repetido en el acuerdo exige un código del MISMO CLIENTE
  -- que ya tiene código en otra posición del SKU. Incluye posiciones excluidas
  -- (spec 03.21.00 §4.6 — conservan identidad).
  if v_product_id is not null then
    ...
    if v_has_other then
      -- Rama A: alguna existente sin código vigente → bloqueo total.
      select exists(
        select 1 from public.agreement_positions ap
         where ap.agreement_id = p_agreement_id
           and ap.product_id   = v_product_id
           and not exists (
             select 1 from public.agreement_position_client_codes apcc
              where apcc.agreement_position_id = ap.id
                and apcc.valid_until is null
           )
      ) into v_codeless_exists;

      if v_codeless_exists then
        raise exception 'Este SKU ya está en el acuerdo sin código de cliente. Asigna primero un código a esa posición.'
          using errcode = 'P0001';
      end if;

      -- Rama B: todas las existentes tienen código. Exigir código de un cliente
      -- que ya tenga código en otra posición del SKU.
      select coalesce(array_agg(distinct apcc.client_id), '{}'::uuid[])
        into v_clients_with_code
      ...
      select coalesce(array_agg(distinct (e->>'client_id')::uuid), '{}'::uuid[])
        into v_new_code_clients
        from jsonb_array_elements(v_codes) e;

      if not (v_new_code_clients && v_clients_with_code) then
        select string_agg(coalesce(c.commercial_name, c.legal_name), ', '
                          order by coalesce(c.commercial_name, c.legal_name))
          into v_client_names
          from public.clients c
         where c.id = any(v_clients_with_code);
        raise exception 'Este SKU ya está en el acuerdo con código de %. Para crear otra posición, % debe nombrarla con un código distinto.',
          v_client_names, v_client_names using errcode = 'P0001';
      end if;
    end if;
  end if;
...
begin
  ...
  -- GUARD por cliente (espejo): si hay otras posiciones del mismo SKU en el
  -- acuerdo, esta posición debe tener código de al menos uno de los clientes
  -- que ya tienen código en esas otras posiciones. Se calcula después de
  -- aplicar el diff, sobre el estado final en DB.
  if v_new_product_id is not null then
    select exists(
      select 1 from public.agreement_positions ap
       where ap.agreement_id = v_agreement_id
         and ap.product_id   = v_new_product_id
         and ap.id           <> p_line_id
    ) into v_has_other;

    if v_has_other then
      -- Alguna OTRA posición del SKU sin código vigente → bloquear.
      select exists(
        select 1 from public.agreement_positions ap
         where ap.agreement_id = v_agreement_id
           and ap.product_id   = v_new_product_id
      ...
      -- Clientes con código vigente en OTRAS posiciones del SKU.
      select coalesce(array_agg(distinct apcc.client_id), '{}'::uuid[])
        into v_clients_with_code
        from public.agreement_position_client_codes apcc
        join public.agreement_positions ap on ap.id = apcc.agreement_position_id
      ...
      -- Clientes con código vigente en ESTA posición (tras el diff).
      select coalesce(array_agg(distinct apcc.client_id), '{}'::uuid[])
        into v_self_code_clients
        from public.agreement_position_client_codes apcc
       where apcc.agreement_position_id = p_line_id
         and apcc.valid_until is null;

      if not (v_self_code_clients && v_clients_with_code) then
        select string_agg(coalesce(c.commercial_name, c.legal_name), ', '
                          order by coalesce(c.commercial_name, c.legal_name))
          into v_client_names
          from public.clients c
         where c.id = any(v_clients_with_code);
        raise exception 'Este SKU ya está en el acuerdo con código de %. La posición debe tener un código de % que la distinga.',
          v_client_names, v_client_names using errcode = 'P0001';
      end if;
    end if;
  end if;
