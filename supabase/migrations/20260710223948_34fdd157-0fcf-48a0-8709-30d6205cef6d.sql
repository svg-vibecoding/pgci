...
-- Eliminar firma antigua con p_kind
DROP FUNCTION IF EXISTS public.update_agreement_line(uuid, text, jsonb, boolean);

CREATE OR REPLACE FUNCTION public.update_agreement_line(
  p_line_id uuid,
  p_patch jsonb,
  p_confirm_n_conflict boolean DEFAULT false
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
